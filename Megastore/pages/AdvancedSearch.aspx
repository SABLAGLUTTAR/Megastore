...
<asp:Content ID="Content4" ContentPlaceHolderID="contentBody" runat="server">
    ...
    <div>
        ...
        <asp:UpdatePanel runat="server" ID="updatePanelIndex">
            <ContentTemplate>
                <asp:GridView ID="GridView1" ShowHeader="False" CssClass="table" Font-Bold="true" AllowSorting="true" GridLines="None" runat="server" Width="100%" CellPadding="5" DataSourceID="SqlDataSource1" AutoGenerateColumns="false" BorderStyle="None">
       <EmptyDataTemplate>There where no products found!</EmptyDataTemplate>
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:dataConn %>" ProviderName="<%$ ConnectionStrings:dataConn.ProviderName %>" SelectCommand="SELECT product_type.type_name AS Type, product_id AS Id,product.product_name AS Name, categories.category_name AS Category, product.product_description AS Description, product.price_per_unit AS Price, product.unit AS Unit, product.image_url AS Image FROM categories INNER JOIN product ON categories.categories_id = product.categories_catogories_id INNER JOIN product_type ON product.product_type_idproduct_type = product_type.product_type_id WHERE (product_type.type_name =@category AND product.product_name LIKE  @name )">
            <SelectParameters>
